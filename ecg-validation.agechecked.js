...
function validateAgeVsRecordingDate() {
    ...
    if (!isNaN(age) && (age < 0 || age > 120)) {
        const msg = `سن وارد شده (${age}) معتبر نیست. لطفاً سنی بین 0 تا 120 سال وارد کنید.`;
    ...
    if (!isNaN(age) && dateInput.value) {
        ...
        let actualAge = today.getFullYear() - estimatedBirthDate.getFullYear();
        const monthDiff = today.getMonth() - estimatedBirthDate.getMonth();
        const dayDiff = today.getDate() - estimatedBirthDate.getDate();
        ...
        if (actualAge - age > 1) {
            ...
            const msg = `تاریخ ثبت (${recordingDate.toLocaleDateString('fa-IR')}) نمی‌تواند قبل از تولد تقریبی بیمار (با سن ${age} سال) باشد.`;
            conflictDiv.textContent = msg;
